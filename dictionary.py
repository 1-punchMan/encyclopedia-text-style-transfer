...
class Dictionary(object):
    ...
    def __init__(self, id2word, word2id, counts):
        ...
        self.mask_index = word2id[MASK_WORD]
        self.check_valid()
        self.n_special_words = 4 + SPECIAL_WORDS
    ...
    def check_valid(self):
        ...
        assert self.bos_index == 1
        assert self.eos_index == 2
        ...
        last_count = 1e18
        for i in range(4 + SPECIAL_WORDS, len(self.id2word)):
    ...
    def export(self, path):
        with open(path, 'w', encoding='utf-8') as f:
            for i in range(4 + SPECIAL_WORDS, len(self.id2word)):
                token = self.id2word[i]
                freq = self.counts[token]
                f.write(f"{token} {freq}\n")

    @staticmethod
    def initialize_vocab():
        word2id = {PAD_WORD: 0, BOS_WORD: 1, EOS_WORD: 2, UNK_WORD: 3}
    ...
    @classmethod
    def build_vocab_from_datasets(cls, datasets):
        """
        Create a dictionary from several datasets where the data are tokenized and split by a space.
        args:
            datasets: list of dataset paths
        """
        special_words = cls.initialize_vocab()
        counts = {k: 0 for k in special_words.keys()}
        ...
        word2id = list(counts.items())
        n_spec = len(special_words)
        special_words, word2id = word2id[:n_spec], word2id[n_spec:]
        word2id.sort(key=lambda x: x[1], reverse=True)
        word2id = special_words + word2id
        word2id = {word: i for i, (word, _) in enumerate(word2id)}
        id2word = {v: k for k, v in word2id.items()}
        dico = cls(id2word, word2id, counts)
        logger.info(f"Built a vocabulary of size {len(dico)} from the datasets.")
    ...
    def index_data(path, bin_path, dico):
        ...
        for i, line in enumerate(f):
            ...
            for w in s:
                ...
                if 0 <= word_id < 4 + SPECIAL_WORDS and word_id != dico.unk_index:
            ...
            sentences.append(dico.eos_index)
        f.close()
